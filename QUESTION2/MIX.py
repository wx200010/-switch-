...
Minium_Pmin = 23
MyAlgorithm_error_THRESHOLD = 15.0
MyAlgorithm_MIN_THREADHOLD = 15
...
def Check_Algorithm_And_Change(CAR_DB , MAX_DB):
    if(Algorithm_mode == 1):            # Minium
        return CAR_DB < Minium_Pmin
    elif(Algorithm_mode == 2):          # Best-effort
        return True
    elif(Algorithm_mode == 3):          # Entropy
        return MAX_DB - CAR_DB > Entropy_THRESHOLD
    elif(Algorithm_mode == 4):          # MyAlgorithm
...
def Update_Connect():
    for car in CAR_array:        
        ...
        if (BS_index != car.connect):
            if(Check_Algorithm_And_Change(car.DB , MAX_DB)):
                if(car.connect != -1):
                    global total_switch
                    total_switch += 1
                car.connect = BS_index
                car.DB = Receive_DB
                car.color = BS_array[BS_index].color
...
def Update_Text():
    ...
    text = "Lambda = " + str(round(LAMBDA*12*5 , 2)) + " cars / min"
    draw_text(text , 40 , MAP_SIZE[0] + 250 , 370 , BLACK)
    
    if(Algorithm_mode == 1):
        text = "Algorithm : Minium"
        ...
        text = "Algorithm : My"
    draw_text(text , 40 , MAP_SIZE[0] + 250 , 500 , BLACK)
